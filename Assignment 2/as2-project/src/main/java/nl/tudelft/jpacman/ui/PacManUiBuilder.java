...
public class PacManUiBuilder {
	...
	/**
	 * Adds a button with the caption {@value #FREEZE_CAPTION} that freeze the
	 * game.
	 * 
	 * @param game
	 *            The game to freeze.
	...
	/**
	 * Adds a button with the caption {@value #UNFREEZE_CAPTION} that unfreeze the
	 * game.
	 * 
	 * @param game
	 *            The game to unfreeze.
	 */
	private void addUnfreezeButton(final Game game) {
		assert game != null;
		
		buttons.put(UNFREEZE_CAPTION, new Action() {
			@Override
			public void doAction() {
				game.unfreeze();
			}
